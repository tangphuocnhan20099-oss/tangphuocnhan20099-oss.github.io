...
const avatarOptions = ["😀", "😎", "🤠", "🥷", "🧙", "🧛", "🦸", "👻", "🤖", "🐲", "🍀", "🔥"];
const maxRouletteHp = 3;
const rouletteItemIds = ["cigarette", "scissors", "cuffs", "beer"];
const suits = [
  { symbol: "♠", color: "black" },
  { symbol: "♥", color: "red" },
  { symbol: "♦", color: "red" },
...
async function handleApi(req, res, url) {
  ...
  if (req.method === "POST" && url.pathname === "/api/roulette/rooms") {
    const body = await readJson(req);
    const player = makePlayer(body);
    const room = createRouletteRoom(player);
    sendJson(res, 200, { playerId: player.id, room: serializeRouletteRoom(room, player.id) });
    return;
  }
  ...
  if (parts[1] === "roulette") {
    await handleRouletteApi(req, res, url, parts);
    return;
  }
...
async function handleRouletteApi(req, res, url, parts) {
  const code = parts[3]?.toUpperCase();
  const room = code ? rouletteRooms.get(code) : null;
  if (!room) {
    throw httpError(404, "Không tìm thấy phòng.");
  }
  ...
  if (req.method === "GET" && parts.length === 4) {
    const playerId = url.searchParams.get("playerId");
    touchPlayer(room, playerId);
    pruneRouletteOfflinePlayers(room);
    sendJson(res, 200, { room: serializeRouletteRoom(room, playerId) });
    return;
  }
  ...
  if (req.method === "POST" && parts[4] === "start") {
    const body = await readJson(req);
    requireHost(room, body.playerId);
    startRouletteRound(room);
    sendJson(res, 200, { room: serializeRouletteRoom(room, body.playerId) });
    return;
  }

  if (req.method === "POST" && parts[4] === "target") {
    const body = await readJson(req);
    touchPlayer(room, body.playerId);
    selectRouletteTarget(room, body.playerId, body.targetPlayerId);
    sendJson(res, 200, { room: serializeRouletteRoom(room, body.playerId) });
    return;
  }

  if (req.method === "POST" && parts[4] === "item") {
    const body = await readJson(req);
    touchPlayer(room, body.playerId);
    useRouletteItem(room, body.playerId, Number(body.itemIndex));
    sendJson(res, 200, { room: serializeRouletteRoom(room, body.playerId) });
    return;
  }

  if (req.method === "POST" && parts[4] === "shoot") {
    const body = await readJson(req);
    touchPlayer(room, body.playerId);
    shootRoulette(room, body.playerId);
    sendJson(res, 200, { room: serializeRouletteRoom(room, body.playerId) });
    return;
  }
  ...
  if (req.method === "POST" && parts[4] === "kick") {
    const body = await readJson(req);
    requireHost(room, body.playerId);
    kickRoulettePlayer(room, body.targetPlayerId);
    sendJson(res, 200, { room: serializeRouletteRoom(room, body.playerId) });
    return;
  }
  ...
  if (req.method === "POST" && parts[4] === "offline") {
    const body = await readJson(req);
    markRoulettePlayerOffline(room, body.playerId);
    sendJson(res, 200, { room: serializeRouletteRoom(room, body.playerId) });
    return;
  }
...
function createRoom(player) {
  ...
  } while (rooms.has(code) || xidachRooms.has(code) || rouletteRooms.has(code));
...
function createRouletteRoom(player) {
  let code = "";
  do {
    code = Math.random().toString(36).slice(2, 6).toUpperCase();
  } while (rooms.has(code) || xidachRooms.has(code) || rouletteRooms.has(code));
  ...
  const room = {
    ...
    roundNumber: 0,
    game: null,
    message: "Phòng Cò quay nga đã tạo. Gửi mã cho bạn bè để vào."
  };
  rouletteRooms.set(code, room);
  return room;
}
...
function startRouletteRound(room) {
  if (room.players.length < 2) throw httpError(400, "Cần ít nhất 2 người chơi.");
  if (room.players.length > 10) throw httpError(400, "Cò quay nga online hiện hỗ trợ tối đa 10 người chơi.");
  ...
  reloadRouletteShotgun(game);
  room.game = game;
  room.phase = "playing";
  room.roundNumber += 1;
  room.message = "";
  prepareRouletteTurn(room, "");
}

function selectRouletteTarget(room, playerId, targetPlayerId) {
  const game = requireRouletteGame(room);
  const shooter = requireCurrentRoulettePlayer(game, playerId);
  const target = game.players.find((player) => player.id === targetPlayerId);
  if (!target?.alive) throw httpError(400, "Người được chọn không còn trong bàn.");
  game.selectedTargetId = target.id;
  room.message = "";
  return shooter;
}

function useRouletteItem(room, playerId, itemIndex) {
  const game = requireRouletteGame(room);
  const shooter = requireCurrentRoulettePlayer(game, playerId);
  if (!Number.isInteger(itemIndex) || itemIndex < 0 || itemIndex >= shooter.items.length) {
    throw httpError(400, "Vật phẩm không hợp lệ.");
  }

  const itemId = shooter.items[itemIndex];
  const selectedTarget = game.players.find((player) => player.id === game.selectedTargetId);
  if (itemId === "scissors" && shooter.powerShot) throw httpError(400, "Bạn đã cắt nòng súng rồi.");
  if (itemId === "cuffs" && (!selectedTarget?.alive || selectedTarget.id === shooter.id)) {
    throw httpError(400, "Còng tay cần chọn người chơi khác còn sống.");
  }

  if (itemId === "cigarette") {
    shooter.hp = Math.min(maxRouletteHp, shooter.hp + 1);
    room.message = `${shooter.name} hồi 1 máu.`;
  }

  if (itemId === "scissors") {
    shooter.powerShot = true;
    room.message = `${shooter.name} đã cắt nòng súng.`;
  }

  if (itemId === "cuffs") {
    selectedTarget.cuffedTurns += 1;
    room.message = `${selectedTarget.name} bị còng tay.`;
  }

  if (itemId === "beer") {
    if (game.shells.length === 0) reloadRouletteShotgun(game);
    game.peek = game.shells[0]
      ? { id: makeId(), playerId: shooter.id, shell: game.shells[0] }
      : null;
    room.message = `${shooter.name} uống bia.`;
  }
  ...
  game.lastItem = {
    ...
    type: itemId,
    playerId: shooter.id,
    targetId: itemId === "cuffs" ? selectedTarget?.id || null : shooter.id
  };
  shooter.items.splice(itemIndex, 1);
}

function shootRoulette(room, playerId) {
  const game = requireRouletteGame(room);
  const shooter = requireCurrentRoulettePlayer(game, playerId);
  const shooterIndex = game.players.findIndex((player) => player.id === shooter.id);
  const target = game.players.find((player) => player.id === game.selectedTargetId);
  if (!target?.alive) throw httpError(400, "Chọn người còn sống trước khi bắn.");
  if (game.shells.length === 0) reloadRouletteShotgun(game);

  const shell = game.shells.shift() || "blank";
  const isLive = shell === "live";
  const damage = shooter.powerShot ? 2 : 1;
  const usedPowerShot = shooter.powerShot;
  let killed = false;

  shooter.powerShot = false;
  game.peek = null;
  if (isLive) game.liveCount = Math.max(0, game.liveCount - 1);
  else game.blankCount = Math.max(0, game.blankCount - 1);
  ...
  if (aliveRoulettePlayers(game).length <= 1) {
    finishRouletteRound(room);
    return;
  }

  if (game.shells.length === 0) reloadRouletteShotgun(game);
  const selfBlankBonus = !isLive && shooter.id === target.id && shooter.alive;
  game.currentIndex = selfBlankBonus ? shooterIndex : nextAliveRouletteIndex(game, shooterIndex);
  room.message = "";
  prepareRouletteTurn(room, "");
}

function kickRoulettePlayer(room, targetPlayerId) {
  if (targetPlayerId === room.hostId) throw httpError(400, "Không thể kick chủ phòng.");
  const playerIndex = room.players.findIndex((player) => player.id === targetPlayerId);
  if (playerIndex === -1) throw httpError(404, "Không tìm thấy người chơi để kick.");
  const [removedPlayer] = room.players.splice(playerIndex, 1);
  removeRouletteGamePlayer(room, targetPlayerId);
  room.message = `${removedPlayer.name} đã bị kick khỏi phòng.`;
}

function markRoulettePlayerOffline(room, playerId) {
  const player = markPlayerOffline(room, playerId);
  if (room.phase === "playing") {
    removeRouletteGamePlayer(room, playerId);
  }
  return player;
}

function pruneRouletteOfflinePlayers(room) {
  if (room.phase !== "playing" || !room.game) return;
  const now = Date.now();
  room.players
    .filter((player) => !isPlayerOnline(player, now))
    .forEach((player) => removeRouletteGamePlayer(room, player.id));
}

function removeRouletteGamePlayer(room, playerId) {
  if (!room.game) {
  ...
  const currentId = game.players[game.currentIndex]?.id || "";
  const removedIndex = game.players.findIndex((player) => player.id === playerId);
  game.players = game.players.filter((player) => player.id !== playerId);
  room.players = room.players.filter((player) => player.id !== playerId);
  if (room.hostId === playerId) room.hostId = room.players[0]?.id || "";

  if (!game.players.length || room.players.length < 2) {
    room.phase = "lobby";
    room.game = null;
    room.message = "Ván đã dừng vì không đủ người chơi.";
    return;
  ...
  if (game.selectedTargetId === playerId) {
    game.selectedTargetId = null;
  }

  const alivePlayers = aliveRoulettePlayers(game);
  ...
  if (currentId && currentId !== playerId) {
    game.currentIndex = Math.max(0, game.players.findIndex((player) => player.id === currentId));
  } else {
    game.currentIndex = Math.min(Math.max(0, removedIndex), game.players.length - 1);
  }
  prepareRouletteTurn(room, "");
}

function requireRouletteGame(room) {
  if (room.phase !== "playing" || !room.game || room.game.gameOver) {
    throw httpError(400, "Ván Cò quay nga chưa bắt đầu.");
  }
  return room.game;
}

function requireCurrentRoulettePlayer(game, playerId) {
  const shooter = game.players[game.currentIndex];
  if (!shooter?.alive) throw httpError(400, "Chưa tới lượt bắn.");
  if (shooter.id !== playerId) throw httpError(403, "Chỉ người đang cầm súng được thao tác.");
  return shooter;
}

function reloadRouletteShotgun(game) {
  const size = randomInt(4, 6);
  const liveCount = randomInt(1, size - 1);
...
function prepareRouletteTurn(room, baseMessage = "") {
  const game = room.game;
  if (!game || game.gameOver) return;
  game.peek = null;

  if (!game.players[game.currentIndex]?.alive) {
    game.currentIndex = nextAliveRouletteIndex(game, game.currentIndex);
  }

  for (let guard = 0; guard < game.players.length; guard += 1) {
    const player = game.players[game.currentIndex];
    if (!player?.alive) {
      game.currentIndex = nextAliveRouletteIndex(game, game.currentIndex);
      continue;
    }
    if (player.cuffedTurns <= 0) break;
    player.cuffedTurns -= 1;
    game.currentIndex = nextAliveRouletteIndex(game, game.currentIndex);
  }

  const shooter = game.players[game.currentIndex];
  game.selectedTargetId = shooter?.alive ? shooter.id : null;
  room.message = baseMessage;
}

function finishRouletteRound(room) {
  const game = room.game;
  if (!game) return;
  const winner = aliveRoulettePlayers(game)[0] || null;
  game.gameOver = true;
  game.winnerId = winner?.id || null;
  game.selectedTargetId = winner?.id || null;
  room.phase = "finished";
  room.message = winner ? `${winner.name} thắng!` : "Không có người thắng.";
}

function aliveRoulettePlayers(game) {
  return game.players.filter((player) => player.alive);
}

function nextAliveRouletteIndex(game, fromIndex) {
  ...
  return Math.max(0, Math.min(fromIndex, game.players.length - 1));
}

function drawRouletteItems(count) {
  return Array.from({ length: count }, () => rouletteItemIds[randomInt(0, rouletteItemIds.length - 1)]);
}

function serializeRouletteRoom(room, viewerId) {
  const now = Date.now();
  const livePlayers = new Map(room.players.map((player) => [player.id, player]));
  return {
    code: room.code,
    ...
    game: room.game ? serializeRouletteGame(room.game, viewerId, livePlayers, now) : null
  };
}

function serializeRouletteGame(game, viewerId, livePlayers, now) {
  const currentPlayer = game.players[game.currentIndex] || null;
  return {
    roundId: game.roundId,
    currentPlayerId: currentPlayer?.id || null,
    selectedTargetId: game.selectedTargetId,
    shellCount: game.shells.length,
    shells: game.shells.map(() => "unknown"),
    beerRevealShell: game.peek?.playerId === viewerId ? game.peek.shell : null,
    beerRevealId: game.peek?.playerId === viewerId ? game.peek.id : null,
    lastItem: game.lastItem,
    lastShot: game.lastShot,
    winnerId: game.winnerId,
    gameOver: game.gameOver,
    canAct: currentPlayer?.id === viewerId && !game.gameOver,
    players: game.players.map((player) => ({
      ...
      hp: player.hp,
      alive: player.alive,
      cuffedTurns: player.cuffedTurns,
      powerShot: player.powerShot,
      items: player.id === viewerId ? player.items : []
    }))
  };
}
